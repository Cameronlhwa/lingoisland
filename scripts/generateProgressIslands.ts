...
// Build prompt for each progress stage
const buildPromptForStage = (stageDescription: string) =>
  `Using the provided image, change ONLY the objects on top of the island surface to represent this progress stage: "${stageDescription}". In this artstyle, keep it simple, clean, and cartoonish. The capybara should be a cute fat furry very cute capybara (light brown caramel colour) standing on two feet. Please maintain the thickness that the original island has for any new graphics on it. Keep the island shape, ocean, sky, lighting, shadows, and composition EXACTLY the same. Do not regenerate or redraw the base island - just add themed elements on its surface. Match the existing art style. You can add one to two plain colours maximum. Try to use a bit of navy blue #182545 (RGB 24, 37, 69)`

// 5 Progress stages for the island
const PROGRESS_STAGES = [
  {
    stage: 1,
    description: 'A barren village island with a sad, poor-looking capybara in tattered clothes, empty buildings, wilted plants',
    filename: 'stage-1.png',
  },
  {
    stage: 2,
    description: 'A developing island with a slightly happier capybara in simple clothes, some small structures being built, a few growing plants',
    filename: 'stage-2.png',
  },
  {
    stage: 3,
    description: 'A growing island with a capybara in decent clothes, multiple structures, gardens starting to bloom',
    filename: 'stage-3.png',
  },
  {
    stage: 4,
    description: 'A thriving island with a happy capybara in nice clothes, well-developed buildings, lush gardens',
    filename: 'stage-4.png',
  },
  {
    stage: 5,
    description: 'A prosperous developed island with a very happy capybara in a professional suit, impressive buildings, beautiful flourishing gardens',
    filename: 'stage-5.png',
  },
]
...
// Generate one progress island image
async function generateOneProgressIsland(
  ...
  const contents = [
    {
      parts: [
        ...
        { text: buildPromptForStage(stageDescription) },
      ],
    },
  ]
  ...
  throw new Error(`Image model returned no image${blockMsg}. Try again or use a different stage description.`)
}

async function main() {
  console.log('🏝️  Progress Islands Generator')
  console.log('=' .repeat(60))
  console.log('Generating 5 progress stage island images using NanoBanana Pro')
  ...
  const outputDir = path.join(process.cwd(), 'public', 'progress-islands')
  ...
  for (let i = 0; i < PROGRESS_STAGES.length; i++) {
    ...
    console.log(`[Stage ${stage.stage}/${PROGRESS_STAGES.length}] ${stage.description}`)
    console.log(`  📝 Filename: ${stage.filename}`)

    try {
      // Generate with retries
      const imageData = await retryWithBackoff(
        () => generateOneProgressIsland(stage.description, baseImageBase64, apiKey),
        2,
        2000
      )
  ...
  if (failCount > 0) {
    console.log('⚠️  Some images failed to generate. Re-run the script to retry failed stages.')
  }
}
